...
/*
  //////////////////////////////////////////////////
  Hybrid_Code_v6 as of 06/28/18:

  TODO:
  verify camera integration with masterstatus variables (fix photosize2 issue)
  fault handling should tell which fault has occured
...
const int faultSize = 10; // always make a multiple of 8
...
class masterStatus {
  ...
  public:
    ...
    int overdown; //this and the next two ints are used by overwrite commands
    int overup;
    int overmode;
    int State; // what does this do
    int NextState; // I don't think we need the state stuff
    int Faults; //number of faults detected over satellite lifetime
    int activeFaults[faultSize]; //What faults are we checking the state of
    String FaultString; // turn activefaults into a string for easy transmitting
    ...
    int numPhotos;
    bool CameraBurst;
    int imageSize; //this is our photosize/photosize2
    ...
    int Temp;
    int TempAccumulator;
    int DoorButton; // what is the default reading of a button
    int lightSense;
    float SolarCurrent;
    float CurrentZero;
    ...
    int downlinkPeriod; // how many millis to wait between each downlink
    int uplinkPeriod; // how many millis to wait before each uplink
    int SRFreq; //how many regular downlinks between special reports
    int SRTimeout;
    uint8_t VarHolder[13]; //I still dont get this
    ...
    masterStatus() {
      ...
      GyroAve[3] = {0}; // sent to us by slave comp
      MagAve[3] = {0}; // sent to us by slave comp
      AccelAve[3] = {0}; // sent to us by slave comp
      ...
      downlinkPeriod = 900000; // how many millis to wait between each downlink
      uplinkPeriod = 60000; // how many millis to wait before each uplink
...
void MLowPower() {
  ...
  //change rockbloc frequency //but already done with powerman?
}
...
void faultCheck() { // todo make this track which fault was tripped
    ...
    if (CurInt = 0) {
      ...
      switch (f) { //this does not increment f //cases can neverthless be added
        ...
        case (3):
          // camera active?
          if (!CamActive) {
            faultReport[ReportOpenSpot + 1]  = CamActive;
            fault = true;
          }
          break;
        ...
        case (4):
          ...
          if (SlaveCompFlag) {
            if (SDActive) {
              ...
              faultReport[ReportOpenSpot + 1]  = SDActive;
              fault = true;
            }
          }
          break;
        ...
        case (6):
          if (!IMUActive) {
            faultReport[ReportOpenSpot + 1]  = IMUActive;
            fault = true;
          }
          break;
        ...
        case (7):
          if (!RBActive) {
            faultReport[ReportOpenSpot + 1]  = RBActive;
            fault = true;
          }
        break;
        case (8): //IMU Magnetorquer x off
          if (){
            faultReport[ReportOpenSpot +1] = ;
          }else{
            fault = true
          }
        break;
        ...
        case (21): //Nitinol Wire actiavted, but photoresistor does not detect difference in light
          if (){   //have we failed to open the compartment? Have it take a pic and the rockblock send a picture
            faultReport[ReportOpenSpot +1] = ;
          }else{
            fault = true
          }
  ...
  void buildFaults() { //do i need this (pobably not)
    int f = 0;
    while (f < faultSize) {
  ...
  void FaultString() { //TODO turn into uint array //this is just to count the ammount of faults we have accumulated in one cycle
    int f = 0;
    String g = "";
    while (f < faultSize) {
  ...
  void routineDownlinkData() { //updating the varaible data we receive with downlink, useful to save data in case we need to retrieve it
    ...
    uint8_t tempholder[13] = {MSH.MagAve[0], MSH.MagAve[1], MSH.MagAve[2], MSH.GyroAve[0], MSH.GyroAve[1], MSH.GyroAve[2], MSH.AccelAve[0], MSH.AccelAve[1], MSH.AccelAve[2], MSH.ImuTemp, MSH.Temp, MSH.SolarCurrent, MSH.Battery};
    for (int i = 0; i < 14; i++) {
  ...
  void Downlink() { // outgoing data must be placed in buf before downlink is called
    ...
    if (DLConf == false) {// is there still an outgoing message awaiting confirmation
  ...
  void DownlinkCheck() { // Should now be finished
    ...
    if (x == ISBD_SUCCESS && imgsent) { //imgsent is a boolean used to verify if the last msg sent was a segment of our image
      ...
      piccounter += min(340, bytesleft); //advances the beginning of the copy from array a to buf
      bytesleft -= min(340, bytesleft); //tells us how much of th buf will be busy with bytes from image //bytesleft, imgsent, and counter should only be changed by the checker, guaranteeing we are always sending fresh bytes at the right time
  ...
  void Uplink() { // this is difficult to develop without a working rockblock
    ...
    delay(10); // is this enough time to confirm weather we've got incoming data?
  ...
  void checkTemp() {
    MSH.Temp = analogRead(A1);
  }

  void currentSense() {// this value will need to be zeroed before practical use
    MSH.SolarCurrent = analogRead(9);
  }
  ...
  void sensorInit() { //TODO Ensure that every sensor input is declared an input here
    SDInit();
    pinMode(A2, INPUT);
    pinMode(13, INPUT);
    pinMode(9, INPUT);
